...
export default function RegisterPage() {
  ...
  const handleRegisterToSupabase = async () => {
    ...
    try {
      ...
      if (selectedReceiptIds.length === 0) {
        throw new Error("登録するアイテムを選択してください");
      }
      ...
      setRegisterStatus({ success: 0, failed: 0 });
        ...
        try {
          ...
          const { error } = await supabase
            ...
            .update({
              ...
              type: selectedDocType || receipt.type,
              memo: receipt.memo,
              tag: receipt.tag,
          ...
          successCount++;
          setRegisterStatus({ success: successCount, failed: failedCount });
        } catch (err) {
          failedCount++;
          setRegisterStatus({ success: successCount, failed: failedCount });
          console.error(`Failed to register receipt ${receipt.id}:`, err);
        }
      }

      if (failedCount === 0) {
        alert(`登録完了: ${successCount}件のアイテムを登録しました`);
      } else {
        alert(
          `処理完了: ${successCount}件成功, ${failedCount}件失敗しました。詳細はコンソールを確認してください。`
        );
      }
    } catch (err) {
  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      <h1 className="text-xl font-bold mb-4">Supabase登録</h1>
      ...
      {isConfirmDialogOpen && (
        <ConfirmDialog
          ...
          }件のレシートをSupabaseに登録します。続行しますか？`}
          confirmLabel="登録する"
          cancelLabel="キャンセル"
          onConfirm={handleRegisterToSupabase}
          onCancel={() => setIsConfirmDialogOpen(false)}
          extraContent={
            <div className="mb-4 p-3 bg-blue-50 text-blue-700 rounded">
              <p className="font-medium">登録情報:</p>
          ...
          >
            {isRegistering ? (
              <span className="flex items-center">
                ...
                登録中... {registerStatus.success}/
                {registerStatus.success + registerStatus.failed}
              </span>
            ) : (
              "Supabaseに登録"
            )}
